...
async function runDailyAutoGeneration() {
  ...
  const prompt = `Ти — експерт із преміального SEO-копірайтингу та локальний експерт туристичного порталу "Гід Трускавця". 
  Твоє завдання — написати корисну, фактологічну статтю українською мовою, яка інтегрує поточний пошуковий тренд "${currentTrendKeyword}" у контекст відпочинку та оздоровлення на Західній Україні (Трускавець, Східниця, Моршин, Карпати).
  
  СУВОРІ ПРАВИЛА СТИЛЮ ТА КОНТЕНТУ (Для відповідності Google E-E-A-T):
  1. ЖОДНОЇ ВОДИ ТА КЛІШЕ: Категорично заборонено використовувати пафосні ШІ-фрази: "шалений тренд", "дивовижний регіон", "ідеальна гармонія", "не має аналогів", "кришталеве повітря". Пиши мовою фактів та реальної користі.
  2. ПРИРОДНИЙ ВСТУП: Не починай статтю з привітань на кшталт "Доброго ранку, шановні читачі!". Починай одразу з суті, проблеми або цікавого факту, який пов'язує тренд із відпочинком.
  3. МАТЧАСТИНА ТА КОНКРЕТИКА: Стаття повинна містити реальну практичну інформацію. Замість загальних фраз про "багато санаторіїв", обов'язково згадай конкретні назви відомих готелів чи санаторіїв (наприклад: Mirotel, Rixos-Prykarpattya, Карпати, Шале Грааль, або джерела Східниці / Трускавця), реальні типи мінеральних вод (Нафтуся, Марія) та специфіку їхнього впливу на організм.
  4. ЕКСПЕРТНИЙ ТОН: Пиши від імені локального гіда, який особисто знає кожен куточок регіону. Використовуй маркери досвіду ("за нашими спостереженнями", "місцеві радять", "перевірено на практиці").
  
  Поверни відповідь СУВОРO у форматі JSON об'єкта (БЕЗ маркдауну \`\`\`json, просто чистий текст об'єкта):
  {
    "title": "Привабливий, але реалістичний SEO-заголовок (до 60 символів), який органічно включає тренд або близьку тему",
    "slug": "url-шлях-транслітом-виключно-через-дефіси",
    "excerpt": "Місткий, клікабельний опис до 150 символів для мета-тегу Description з головним ключовим словом. Без штампів.",
    "keywords": "Трускавець, відпочинок Трускавець, санаторії Західної України, Карпати, плюс 2-3 теги тренду",
    "image_keyword": "одне слово англійською для пошуку головного фото в Unsplash (наприклад: 'resort', 'spa', 'forest')",
    "content": "Повний текст статті виключно в HTML форматі. Текст має бути розбитий на логічні блоки. Кожен абзац загорни в <p>, підзаголовки структурних блоків в <h3>, списки з конкретними порадами/локаціями в <ul><li>. Найважливіші факти, назви джерел чи готелів виділяй за допомогою <strong>. Без знаків переносу рядків \\n."
  }`;
