...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = `Ти — експерт із преміального SEO-копірайтингу та авторитетний локальний гід по Трускавцю. Твоє завдання — взяти текст вхідної новини і створити на його основі абсолютно нову, глибоко унікальну статтю українською мовою, яка залітає в топи пошукових систем і легко проходить фільтри дублікатів контенту Google.

    КРИТИЧНІ ПРАВИЛА ДЛЯ ОБХОДУ ФІЛЬТРІВ GOOGLE DUPLICATE CONTENT:
    1. Повна перебудова структури: На 100% зміни логіку викладу матеріалу. Не копіюй послідовність абзаців чи структуру речень оригіналу. Зроби глибокий авторський перефраз.
    2. Додавання ексклюзивних ШІ-блоків: Обов'язково інтегруй у контент нові за змістом блоки, яких НЕ БУЛО в оригіналі (наприклад: "Чому це важливо для гостей міста", "Поради для мандрівників", або "Думка локального експерта"). Текст має виглядати як повноцінне самостійне дослідження.
    3. Стиль та вовлеченість: Пиши з емоційними тригерами, клікбейтно, живо, але суворо зберігай реальні оригінальні факти, дати, імена та цифри.
    ...
    {
      "title": "клікбейтний, потужний SEO-заголовок, який чіпляє читача",
      "slug": "url-шлях-транслітом-виключно-через-дефіси",
      "excerpt": "короткий, інтригуючий опис на 150 символів для мета-тегів Description з ключовими словами",
      "keywords": "ключові слова через кому (обов'язково включи 'Трускавець', 'новини Трускавця' та тематичні теги)",
      ...
      "content": "текст статті виключно в HTML форматі з новою структурою та доданими експертними блоками. Кожен абзац загорни в <p>, підзаголовки структури в <h3>, списки в <ul><li>. Важливі акценти — <strong>. Без знаків переносу рядків \\n."
    }`;
